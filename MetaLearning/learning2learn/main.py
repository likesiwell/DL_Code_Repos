...
def get_inputs(n_dimension, n):
    theta_param = np.random.randn(n_dimension, 1) # parameters
    
    W_inputs = np.random.randn(n, n_dimension, n_dimension) # data
...
def build_optimizer_graph():
    ...
    for i in range(num_of_coordinates):
        cell_list.append(tf.nn.rnn_cell.MultiRNNCell([lstm_cell() for _ in range(num_layers)])) # num_layers = 2 according to the paper.
    batch_size = 1
